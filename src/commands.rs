...
pub async fn unregister_all_guild_application_commands(client: &HttpClient, target_guild_id: GuildId) {
    log::info!("Unregistering all application commands for guild {}", target_guild_id);
...
pub async fn register_global_application_commands(client: &HttpClient) {
    ...
    match client.create_global_application_commands(&serenity::json::Value::from(commands.0)).await{
        Ok(_) => (),
        Err(err) => {
...
pub async fn register_guild_application_commands(client: &HttpClient, target_guild_id: GuildId) {
    ...
    // copied invocation from
    // https://docs.rs/serenity/latest/src/serenity/model/guild/guild_id.rs.html#1456-1469
    match client.create_guild_application_commands(target_guild_id.into(), &serenity::json::Value::from(commands.0)).await{
        Ok(_) => (),
        Err(err) => {
            log::error!("Error creating application commands for guild {}", target_guild_id);
            log::error!("{:?}", err);
        }
    };
}
