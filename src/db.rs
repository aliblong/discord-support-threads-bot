...
pub async fn get_list_of_supported_guilds_ordered_by_id(db_pool: &PgPool)
...
pub async fn update_support_channel_id(db_pool: &PgPool, guild_id: GuildId, support_channel_id: i64) -> Result<()> {
    let raw_guild_id = guild_id.0 as i64;
    let support_channel_id = sqlx::query!(
        "update guilds
...
pub async fn get_support_channel_id(db_pool: &PgPool, guild_id: GuildId) -> Result<Option<GuildId>> {
    let raw_guild_id = guild_id.0 as i64;
    let support_channel_id = sqlx::query!(
